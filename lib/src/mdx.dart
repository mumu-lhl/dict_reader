...
class MDX implements DictReaderBase {
  ...
  Stream<(String, dynamic)> read([bool returnData = false]) async* {
    await _raf.setPosition(_recordBlockOffset);

    final numRecordBlocks = await _readNumberer(_raf);
    // number of entries
    await _readNumberer(_raf);
    ...
    for (var i = 0; i < numRecordBlocks; i++) {
      final compressedSize = await _readNumberer(_raf);
    ...
    var offset = 0;
    var i = 0;
    var recordBlockOffset = await _raf.position();

    for (final compressedSize in recordBlockLnfoList) {
      final recordBlock = _decodeBlock(await _raf.read(compressedSize));
  ...
  Future<Map<String, String>> _readHeader() async {
    var headerBytesSize = await _readNumberer(_raf, 4);

    var contentBytes = await _raf.read(headerBytesSize);
    String content;
  ...
  Future<List<(int, String)>> _readKeys() async {
    await _raf.setPosition(_keyBlockOffset);

    // number of key blocks
    await _readNumberer(_raf);

    // number of entries
    numEntries = await _readNumberer(_raf);

    // number of bytes of key block info after decompression
    if (_version >= 2.0) {
      await _raf.read(_numberWidth);
    }

    // number of bytes of key block info
    final keyBlockInfoSize = await _readNumberer(_raf);
    // number of bytes of key block
    final keyBlockSize = await _readNumberer(_raf);

    if (_version >= 2.0) {
      await _raf.read(4);
    }

    final bytes = await _raf.read(keyBlockInfoSize);
    List<int> keyBlockInfoList = _decodeKeyBlockInfo(bytes);

    // read key block
    final List<int> keyBlockCompressed =
        List.from(await _raf.read(keyBlockSize));

    // extract key block
    final keyList = _decodeKeyBlock(keyBlockCompressed, keyBlockInfoList);

    _recordBlockOffset = await _raf.position();
